// Azure Function - G Travel File Access with Dynamic Statistics (IMPROVED FOR FRONTEND)
...
// IMPROVED: Get available files with better frontend compatibility
        ...
        for (const accno of accNoList) {
            try {
                ...
                if (result.recordset && result.recordset.length > 0) {
                    ...
                    // IMPROVED: Create files structure that matches frontend expectations
                    files.push({
                        id: `${accno}-data`,
                        fileName: `${accno}_InvoiceData_${earliestDate.toISOString().split('T')[0]}.csv`,
                        fileId: `${accno}-data`, // For compatibility
